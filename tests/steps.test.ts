...
describe('every project should be declared in the workspace', () => {
  ...
  it.each(projectsDir)('%s', stepDir => {
    expect(packageJson.steps.includes(stepDir)).toBeTruthy();
  });
});
...
describe.skip("project's package.json name should have same name as the directory", () => {
  const projectsDir = getProjectsDir();
  it.each(projectsDir)('%s', stepDir => {
...
describe("solution's README.md should be identical to exercise's one", () => {
  ...
  it.each(solutionStepsDir)('%s', solutionStepDir => {
    ...
    const solutionReadme = fs.readFileSync(path.resolve(path.join(STEPS_PATH, solutionStepDir, 'README.md')), 'utf-8');
